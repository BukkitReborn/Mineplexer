...
{
  private static String CREATE_IGNORE_TABLE = "CREATE TABLE IF NOT EXISTS accountIgnore (id INT NOT NULL AUTO_INCREMENT, uuidIgnorer VARCHAR(100), uuidIgnored VARCHAR(100));";
  ...
  {
    int rowsAffected = executeUpdate(ADD_IGNORE_RECORD, new Column[] { new ColumnVarChar("name", 100, name), new ColumnVarChar("name", 100, 
      caller.getName()) });
  ...
  {
    int rowsAffected = executeUpdate(DELETE_IGNORE_RECORD, new Column[] { new ColumnVarChar("name", 100, caller), new ColumnVarChar("name", 
      100, name) });
    
    return rowsAffected > 0;
  }
  
  public IgnoreData loadClientInformation(ResultSet resultSet)
    throws SQLException
  {
