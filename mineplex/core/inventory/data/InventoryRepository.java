  ...
  extends RepositoryBase
{
  ...
  {
    executeUpdate(INSERT_ITEM, new Column[] { new ColumnVarChar("name", 100, name), new ColumnInt("categoryId", categoryId) });
  }
  
  public void addCategory(String name)
  {
    executeUpdate(INSERT_CATEGORY, new Column[] { new ColumnVarChar("name", 100, name) });
  }
  ...
  {
    if (executeUpdate(UPDATE_CLIENT_INVENTORY, new Column[] { new ColumnInt("count", count), new ColumnInt("id", accountId), new ColumnInt("itemid", itemId) }) < 1) {
      return executeUpdate(INSERT_CLIENT_INVENTORY, new Column[] { new ColumnInt("id", accountId), new ColumnInt("itemid", itemId), new ColumnInt("count", count) }) > 0;
    }
    return true;
  }
  
  public ClientInventory loadClientInformation(ResultSet resultSet)
    throws SQLException
  {
