  ...
  extends RepositoryBase
{
  ...
  {
    executeUpdate(CREATE_ELO_TABLE, new Column[0]);
  }
  
  public void saveElo(String uuid, String gameType, int elo)
  {
    executeUpdate(INSERT_ELO, new Column[] { new ColumnVarChar("uuid", 100, uuid), new ColumnVarChar("gameType", 100, gameType), new ColumnInt("elo", elo) });
  }
  
  public EloClientData loadClientInformation(ResultSet resultSet)
    throws SQLException
  {
    EloClientData clientData = new EloClientData();
    while (resultSet.next()) {
      clientData.Elos.put(resultSet.getString(1), Integer.valueOf(resultSet.getInt(2)));
    }
    return clientData;
  }
