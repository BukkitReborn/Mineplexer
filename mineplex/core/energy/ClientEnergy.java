...
public class ClientEnergy
{
  ...
  {
    int bonus = 0;
    for (Iterator localIterator = this.MaxEnergyMods.values().iterator(); localIterator.hasNext();)
    {
  ...
  {
    int mod = 0;
    for (Iterator localIterator = this.SwingEnergyMods.values().iterator(); localIterator.hasNext();)
    {
      int i = ((Integer)localIterator.next()).intValue();
      mod += i;
    }
