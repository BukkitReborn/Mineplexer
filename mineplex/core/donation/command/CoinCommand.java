  ...
  extends CommandBase<DonationManager>
{
  ...
  {
    super(plugin, Rank.ADMIN, new String[] { "coin" });
  }
  ...
  public void Execute(final Player caller, String[] args)
  {
    if (args.length < 2)
    {
      UtilPlayer.message(caller, F.main("Shards", "Missing Args: " + F.elem("/coin <player> <amount>")));
      return;
    }
    ...
    if (target == null) {
      ((DonationManager)this.Plugin).getClientManager().loadClientByName(targetName, new Runnable()
      {
        public void run()
        {
          CoreClient client = ((DonationManager)CoinCommand.this.Plugin).getClientManager().Get(targetName);
          if (client != null) {
            CoinCommand.this.rewardCoins(caller, null, targetName, client.getAccountId(), coinsString);
          } else {
