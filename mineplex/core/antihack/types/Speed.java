  ...
  implements Detector
{
  ...
  private void UpdateSpeed(Player player, PlayerMoveEvent event)
  {
    ...
    {
      double offset = event.getFrom().getY() > event.getTo().getY() ? UtilMath.offset2d(event.getFrom(), event.getTo()) : UtilMath.offset(event.getFrom(), event.getTo());
      double limit = 0.74D;
      ...
      count = (offset > limit) && (!UtilTime.elapsed(((Long)((Map.Entry)this._speedTicks.get(player)).getValue()).longValue(), 100L)) ? ((Integer)((Map.Entry)this._speedTicks.get(player)).getKey()).intValue() + 1 : 0;
    }
