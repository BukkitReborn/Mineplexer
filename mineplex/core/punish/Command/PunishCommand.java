  ...
  extends CommandBase<Punish>
{
  ...
  public void Execute(final Player caller, String[] args)
  {
    ...
    else
    {
      ...
      if (target != null)
      {
        ((Punish)this.Plugin).GetRepository().LoadPunishClient(playerName, new Callback()
        {
          public void run(PunishClientToken clientToken)
          {
            ((Punish)PunishCommand.this.Plugin).LoadClient(clientToken);
            new PunishPage((Punish)PunishCommand.this.Plugin, caller, playerName, finalReason);
          }
        });
        return;
      }
