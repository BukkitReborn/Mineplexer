  ...
  extends MapRenderer
{
  ...
  private byte[] colors = new byte[2097152];
  ...
  {
    int i = 1 << this.worldmap.scale;
    int j = this.worldmap.centerX;
    int k = this.worldmap.centerZ;
    int l = MathHelper.floor(0.0D) / i + 64;
    int i1 = MathHelper.floor(0.0D) / i + 64;
    int j1 = 128 / i;
    for (int k1 = l - j1 + 1; k1 < l + j1; k1++)
    {
      ...
      if (l1 <= i2) {
        this.worldmap.flagDirty(k1, l1, i2);
      }
    }
  }
  ...
  {
    int scale = 1 << this.worldmap.scale;
    byte i = 30;
    
    HashMap<Map.Entry, Byte> map = new HashMap();
    for (int mapX = 0; mapX < 128; mapX++) {
      for (int mapZ = 0; mapZ < 128; mapZ++)
      {
        byte color = 0;
        
        int bX = this.worldmap.centerX + (mapX - 64) * scale & 0xF;
        int bZ = this.worldmap.centerZ + (mapZ - 64) * scale & 0xF;
        if ((bX == 0) || (bX == 15) || (bZ == 0) || (bZ == 15))
        {
          Map.Entry<Integer, Integer> entry = new AbstractMap.SimpleEntry(Integer.valueOf((mapX - bX) / 16), Integer.valueOf((mapZ - bZ) / 16));
          if (!map.containsKey(entry)) {
            map.put(entry, Byte.valueOf(i++));
          }
          color = ((Byte)map.get(entry)).byteValue();
        }
        else
        {
          color = this.colors[(mapZ * 128 + mapX)];
        }
