  ...
  extends MiniPlugin
{
  ...
  private NautHashMap<Player, VisibilityData> _data = new NautHashMap();
  ...
  {
    if (!this._data.containsKey(player)) {
      this._data.put(player, new VisibilityData());
    }
    return (VisibilityData)this._data.get(player);
  }
  ...
  public void setVisibility(Player target, boolean isVisible, Player... viewers)
  {
    ...
    int j = (arrayOfPlayer = viewers).length;
    for (int i = 0; i < j; i++)
    {
  ...
  public void update(UpdateEvent event)
  {
    ...
    Iterator<Player> playerIter = this._data.keySet().iterator();
    while (playerIter.hasNext())
    {
      ...
      if ((!player.isOnline()) || (!player.isValid())) {
        ...
        ((VisibilityData)this._data.get(player)).attemptToProcessUpdate(player);
      }
    }
  ...
  {
    this._data.remove(event.getPlayer());
  }
