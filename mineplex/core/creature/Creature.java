  ...
  extends MiniPlugin
{
  ...
  public void Death(EntityDeathEvent event)
  {
    ...
    else if (event.getEntityType() == EntityType.PIG_ZOMBIE)
    {
      ...
      if (UtilMath.r(100) > 90) {
        drops.add(ItemStackFactory.Instance.CreateStack(Material.GOLD_SWORD, 1));
      }
    }
  ...
  public void UpdateEntityNames(UpdateEvent event)
  {
    ...
    for (Entity ent2 : UtilEnt.GetEntityNames().keySet()) {
      if ((ent2.isDead()) || (!ent2.isValid())) {
        remove.add(ent2);
      }
    }
    for (Entity ent2 : remove) {
      UtilEnt.GetEntityNames().remove(ent2);
    }
  }
