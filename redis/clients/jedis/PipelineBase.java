  ...
  implements BinaryRedisPipeline, RedisPipeline
{
  ...
  {
    getClient(key).del(new String[] { key });
    return getResponse(BuilderFactory.LONG);
  }
  
  public Response<Long> del(byte[] key)
  {
    getClient(key).del(new byte[][] { key });
    return getResponse(BuilderFactory.LONG);
  }
  ...
  {
    getClient(key).zrevrangeByScoreWithScores(key, Protocol.toByteArray(max), Protocol.toByteArray(min), offset, count);
