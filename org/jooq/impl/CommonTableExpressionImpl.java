...
class CommonTableExpressionImpl<R extends Record>
  ...
  implements CommonTableExpression<R>
{
  ...
  {
    if (ctx.declareCTE()) {
      ctx.visit(this.name).sql(" ").keyword("as").sql(" (").formatIndentStart().formatNewLine().visit(this.select).formatIndentEnd().formatNewLine().sql(")");
    } else {
      ctx.visit(DSL.name(new String[] { this.name.name }));
    }
  }
  ...
  final Fields<R> fields1()
  {
    ...
    for (int i = 0; i < f.length; i++) {
      f[i] = DSL.fieldByName(
        ((Field)s.get(i)).getDataType(), new String[] { this.name.name, this.name.fieldNames.length > 0 ? this.name.fieldNames[i] : 
        
        ((Field)s.get(i)).getName() });
    }
