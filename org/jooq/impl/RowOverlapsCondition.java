...
class RowOverlapsCondition<T1, T2>
  extends AbstractCondition
{
  ...
  private final QueryPartInternal delegate(Configuration configuration)
  {
    ...
    {
      if (intervalOverlaps) {
        return (QueryPartInternal)right1.le(left1.add(left2)).and(left1
          .le(right1.add(right2)));
      }
      return (QueryPartInternal)right1.le(left2.cast(right1)).and(left1
        .le(right2.cast(left1)));
    }
    if (intervalOverlaps) {
      if (Arrays.asList(new SQLDialect[] { SQLDialect.HSQLDB }).contains(configuration.dialect())) {
        return (QueryPartInternal)right1.le(left1.add(left2)).and(left1
          .le(right1.add(right2)));
      }
    }
