...
class UpdateQueryImpl<R extends Record>
  ...
  implements UpdateQuery<R>
{
  ...
  UpdateQueryImpl(Configuration configuration, Table<R> table)
  {
    ...
    this.updateMap = new FieldMapForUpdate(Clause.UPDATE_SET_ASSIGNMENT);
  ...
  public final void addFrom(Collection<? extends TableLike<?>> f)
  {
    for (TableLike<?> provider : f) {
      this.from.add(provider.asTable());
    }
  }
  ...
  {
    ctx.start(Clause.UPDATE_UPDATE).keyword("update").sql(" ").declareTables(true).visit(getInto()).declareTables(false).end(Clause.UPDATE_UPDATE);
    
    ctx.formatSeparator()
      .start(Clause.UPDATE_SET)
      .keyword("set")
      .sql(" ");
    if (this.multiRow != null)
    {
      boolean qualify = ctx.qualify();
      
      ctx.start(Clause.UPDATE_SET_ASSIGNMENT)
        .qualify(false)
        .visit(this.multiRow)
        .qualify(qualify)
        .sql(" = ");
      if ((this.multiValue != null) && (!Arrays.asList(new Object[0]).contains(ctx.configuration().dialect().family())))
      {
        ctx.visit(this.multiValue);
      }
      else
      {
        Select<?> select = this.multiSelect;
        if (this.multiValue != null) {
          select = DSL.select(this.multiValue.fields());
        }
        ctx.sql("(").formatIndentStart().formatNewLine().subquery(true).visit(select).subquery(false).formatIndentEnd().formatNewLine().sql(")");
      }
      ctx.end(Clause.UPDATE_SET_ASSIGNMENT);
    }
    else
    {
      ctx.formatIndentLockStart().visit(this.updateMap).formatIndentLockEnd();
    }
    ctx.end(Clause.UPDATE_SET);
    switch (ctx.configuration().dialect().family())
    {
    }
    ctx.start(Clause.UPDATE_FROM);
    if (!this.from.isEmpty()) {
      ctx.formatSeparator().keyword("from").sql(" ").declareTables(true).visit(this.from).declareTables(false);
    }
    ...
    ctx.end(Clause.UPDATE_WHERE).start(Clause.UPDATE_RETURNING);
  ...
  {
    return (this.updateMap.size() > 0) || (this.multiRow != null);
  }
}
