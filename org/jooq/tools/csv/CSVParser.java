...
public class CSVParser
{
      ...
      else if (c == this.quotechar)
      {
        ...
        else
        {
          if ((!this.strictQuotes) && 
            (i > 2)) {
            if (nextLine.charAt(i - 1) != this.separator) {
              if ((nextLine.length() > i + 1) && (nextLine.charAt(i + 1) != this.separator)) {
  ...
  private boolean isNextCharacterEscapedQuote(String nextLine, boolean inQuotes, int i)
  {
    ...
    return nextLine.charAt(i + 1) == this.quotechar;
  }
  ...
  protected boolean isNextCharacterEscapable(String nextLine, boolean inQuotes, int i)
  {
    ...
    return (nextLine.charAt(i + 1) == this.quotechar) || (nextLine.charAt(i + 1) == this.escape);
  }
